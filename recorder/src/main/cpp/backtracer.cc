...
Backtracer::SymInfoError::SymInfoError(const char* msg) : std::runtime_error(msg) {}
Backtracer::SymInfoError::SymInfoError(const std::string& msg) : std::runtime_error(msg) {}
Backtracer::SymInfoError::~SymInfoError() {}

Backtracer::SymInfoError Backtracer::SymInfoError::err(int err_num) {
    int sz = 1024;
    ...
    if (strerror_r(err_num, buff.get(), sz) != 0) {
        ...
        throw Backtracer::SymInfoError(unknown_error.c_str());
    }
...
public:
    ElfFile(const std::string& path) : path_(path) {
        ...
        if (fd_ == -1) throw Backtracer::SymInfoError::err(errno);
        elf_ = elf_begin(fd_, ELF_C_READ_MMAP, NULL);
        if (elf_ == nullptr) {
            throw Backtracer::SymInfoError("File format not recognizable for: " + path_);
        }
        if (elf_kind(elf_) != ELF_K_ELF) {
            throw Backtracer::SymInfoError("File " + path_ + " is not an elf.");
        }
    }
...
Backtracer::MappedRegion::MappedRegion(Addr start, std::string file): start_(start), file_(file) {
    ElfFile elf_file(file_);
    load_elf(elf_file.get());
}

Backtracer::MappedRegion::~MappedRegion() {}

const std::string Backtracer::MappedRegion::site_for(Addr addr) {
...
Backtracer::SymInfoError Backtracer::MappedRegion::section_error(Elf* elf, size_t shstrndx, Elf_Scn *scn, GElf_Shdr *shdr, const std::string& msg) {
    std::stringstream ss;
...
void Backtracer::MappedRegion::load_symbols(Elf* elf, GElf_Ehdr* ehdr, Elf_Scn* scn, Elf_Scn* xndxscn, GElf_Shdr* shdr) {
    size_t shstrndx;
...
void Backtracer::MappedRegion::load_elf(Elf* elf) {
    GElf_Ehdr ehdr_mem;
...
Backtracer::SymInfo::It Backtracer::SymInfo::region_for(Addr addr) const {
    auto it = mapped.lower_bound(addr);
    it--;
    return it;
}

Backtracer::SymInfo::SymInfo() {
    elf_version(EV_CURRENT);
    dl_iterate_phdr(dyn_link_handler, this);
}

void Backtracer::SymInfo::index(const char* path, Addr start) {
    mapped[start] = std::unique_ptr<MappedRegion>(new MappedRegion(start, path));
}

const std::string& Backtracer::SymInfo::file_for(Addr addr) const {
...
std::string Backtracer::SymInfo::site_for(Addr addr) const {
    auto it = region_for(addr);
    if (it == std::end(mapped)) throw std::runtime_error("can't resolve addresss " + std::to_string(addr));
    return it->second->site_for(addr);
}

void Backtracer::SymInfo::print_frame(Addr pc) const {
...
static int dyn_link_handler(struct dl_phdr_info* info, size_t size, void* data) {
    auto si = reinterpret_cast<Backtracer::SymInfo*>(data);
...
void print_my_bt(Backtracer::SymInfo& sinfo) {
    NativeFrame trace[100];
    auto c = Backtracer::fill_backtrace((NativeFrame*) &trace, 100);
    for (int i = 0; i < c; i++) {
        sinfo.print_frame(trace[i]);
    }
}
