...
public class ZookeeperBasedBackendAssociationStoreTest {
  ...
  public void testReportOfNewBackends(TestContext context) {
    ...
    Future<Recorder.ProcessGroups> f1 = backendAssociationStore.reportBackendLoad(
        BackendDTO.LoadReportRequest.newBuilder().setIp("1").setLoad(0.1f).setPrevTick(Long.MAX_VALUE).setCurrTick(1).build());
    Future<Recorder.ProcessGroups> f2 = backendAssociationStore.reportBackendLoad(
        BackendDTO.LoadReportRequest.newBuilder().setIp("2").setLoad(0.2f).setPrevTick(Long.MAX_VALUE).setCurrTick(1).build());
          ...
          if(ar2.failed()) {
            ...
            vertx.setTimer(3000, timerId -> {
              backendAssociationStore.reportBackendLoad(
                  BackendDTO.LoadReportRequest.newBuilder().setIp("1").setLoad(0.5f).setPrevTick(1).setCurrTick(2).build())
                  .setHandler(ar3 -> {
                if(ar3.failed()) {
                  context.fail(ar3.cause());
                } else {
